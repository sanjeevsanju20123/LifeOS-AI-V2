  ...
  return {
    ...
    averageProgress:
      Object.values(goalProgress).length > 0
        ? Math.round(
            Object.values(goalProgress).reduce(
              (sum, value) => sum + value,
              0
            ) /
              Object.values(goalProgress)
                .length
          )
        : 0,
...
export function generateARESPatternMessages(
  ...
  if (taskBehavior?.strongestPriority) {
    messages.push(
  ...
  if (progressTrend?.trend === "slowing") {
    messages.push(
